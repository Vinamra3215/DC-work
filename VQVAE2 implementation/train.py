...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for batch in pbar:
        ...
        recon_loss = loss_ll + (20.0 * loss_hf)
    ...
    if epoch % SAVE_EVERY == 0 or epoch == NUM_EPOCHS:
        save_path = os.path.join(SAVE_DIR, f"vqvae2_epoch_{epoch}.pth")
        ...
        with torch.no_grad():
            try:
                ...
                recon_rgb = to_rgb((recon_tensor * 2) - 1)
                target_rgb = to_rgb((target_tensor * 2) - 1)
                val_lpips = lpips_metric(recon_rgb, target_rgb).item()

                fig, axs = plt.subplots(1, 3, figsize=(15, 5))

                axs[0].imshow(input_img_np, cmap='gray')
                axs[0].set_title("Input (NSA 1)")
                axs[0].axis('off')

                axs[1].imshow(target_img_np, cmap='gray')
                axs[1].set_title("Target (NSA 3)")
                axs[1].axis('off')

                axs[2].imshow(recon_img_np, cmap='gray')
                axs[2].set_title("Generated")
                axs[2].axis('off')

                title_text = f"Epoch {epoch} | PSNR: {val_psnr:.2f}dB | SSIM: {val_ssim:.4f} | LPIPS: {val_lpips:.4f}"
