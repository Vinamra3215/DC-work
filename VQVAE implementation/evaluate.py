...
SPLIT = "test"
CHECKPOINT_PATH = "./results_vqvae/vqvae_epoch_150.pth"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
OUTPUT_DIR = f"results/vqvae_{SPLIT}_eval"
...
def save_comparison(low, high, recon, diff, idx, psnr, ssim):
    ...
    axs[2].imshow(recon, cmap="gray"); axs[2].set_title(f"VQ-VAE (PSNR: {psnr:.2f})")
...
def main():
    print(f"✅ Evaluating Original VQ-VAE on {SPLIT} set")
    ...
    print("\n" + "="*40)
    print(f"📢 FINAL ORIGINAL VQ-VAE RESULTS ({SPLIT})")
